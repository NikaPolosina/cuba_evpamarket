...
class OrderController extends Controller{
    ...
    public function orderRegistrHandle($id, $shop){
        ...
        $seller = User::where('id', Auth::user()->id)->with([
            'getCompanies' => function ($query) use ($shop){
                $query->where('id', $shop);
            }
        ])->first();
        ...
        return view('order.createHandle')->with('breadcrumbs', $this->_breadcrumbs)->with('seller', $seller)->with('region', $region)->with('city', $city)->with('money', $money)->with('user', $user);
    }
    ...
    public function orderHandleReady(Request $request, $id = NULL){
        ...
        if($id){
            ...
            return view('order.readyHandle')->with('breadcrumbs', $this->_breadcrumbs);
        }
        ...
        $this->validate($request, [
            ...
            'user_id'    => 'required',
            'phone'      => 'required',
        ]);//Проводим валидацию полей. Данные поля должны быть обязательными.
    ...
    public function createOrder(Request $request, CartController $cartController){
        ...
        if($order['discount']){
            $view->with('percent', $order['discount']->percent);
        }else{
            $view->with('percent', 0);
        }
        return $view;
    ...
    public function ready(Request $request){
        ...
        $order = Order::select('order.*', DB::raw('sum(order.total_price) as total_sum'))->where('simple_user_id', $userSeller)->where('owner_user_id', $owner[0]['id'])->where('status', $status[0]['id'])->first();
        ...
        try{
            ...
            $order = Order::create([
                ...
                'percent'        => ($cOrder['discount']) ? $cOrder['discount']->percent : 0,
                'order_phone'    => $request['phone'],
                'region'         => $request['region_id'],
                'city'           => $request['city_id'],
                'street'         => $request['street'],
        ...
        return view('order.ready')->with('breadcrumbs', $this->_breadcrumbs);
    ...
    public function showOrder($id, $status = 0){
        ...
        $myStatus = $company->getOrder()->select([ 'status' ])->groupBy('status')->with([
            'getStatusOwner' => function ($query){
                $query->select([
                    'id',
                    'title'
                ]);
        ...
        return view('order.orderListShop')->with('company', $company)->with('order', $order)->with('myStatus', $myStatus)->with('status', $status)->with('breadcrumbs', $this->_breadcrumbs);
    }
    ...
    public function showSimpleOrder($id){
        ...
        return view('order.simple')->with('order', $order)->with('breadcrumbs', $this->_breadcrumbs);
    }

    public function showSimpleOrderList(){
        $order = Order::where('simple_user_id', '=', Auth::user()->id)->with('getFeedback')->with([
        ...
        $finishOrder = Order::where('simple_user_id', '=', Auth::user()->id)->where('status', $st['0']->id)->with([
            'getProductOrder' => function ($query){
                $query->with('getProductId');
            }
        ])->get();
    ...
    public static function getAmount($companyId, $days){
        $amount = 0;
        $orsders = Company::find($companyId)->getOrder()->where('status', 16)->where('updated_at', '>=', Carbon::today()->subDays($days))->get();
        if(count($orsders)){
